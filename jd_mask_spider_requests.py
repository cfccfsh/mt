...
class Jd_Mask_Spider(object):
    ...
    def make_reserve(self):
        ...
        resp = self.session.get(url=url, params=payload, headers=headers).enconde("utf-8","ignore").decode("latin1")
        print(resp.text)
    ...
    def get_seckill_url(self):
        ...
        while True:
            resp = self.session.get(url=url, headers=headers, params=payload).enconde("utf-8","ignore").decode("latin1")
            print(resp.text)
            resp_json = parse_json(resp.text)
    ...
    def _get_seckill_init_info(self):
        ...
        resp = self.session.post(url=url, data=data, headers=headers).enconde("utf-8","ignore").decode("latin1")
        print(resp.text)
    ...
    def submit_seckill_order(self):
        ...
        resp = self.session.post(
            ...
            data=self.seckill_order_data.get(
                self.sku_id),
            headers=headers).enconde("utf-8","ignore").decode("latin1")
        print(resp.text)
        resp_json = parse_json(resp.text)
        # 返回信息
        # 抢购失败：
